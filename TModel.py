...
class TransformerModel(torch.nn.Module):
    ...
    def Build(self,hiddendim,nhead,num_layers, learningRate=0.001, weight_decay=0.0001):
        '''
        ...
        self.fc_out = nn.Linear(self.hiddendim, self.output_dim).to(self.device, non_blocking=True)
    ...
    def forward(self,xt):
        ...
        xt = self.position_encode(xt)

        trans_out = self.transformer_encoder(xt)

        velocity = self.fc_out(trans_out)
